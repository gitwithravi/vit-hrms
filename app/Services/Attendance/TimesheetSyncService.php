...
class TimesheetSyncService
{
    ...
    private function markAttendanceIfWorkShiftRecordIsNotHoliday(Collection $employeeTimesheets, Carbon $date, array $employeeWorkShiftRecord, array $attendanceTypes, int $employeeId)
    {
        ...
        if ($firstInTime->gt($startTime) && $firstInTime->diffInMinutes($startTime) > config('config.attendance.late_grace_period')) {
            $isLate = true;
        }

        if ($lastOutTime->lt($endTime) && $lastOutTime->diffInMinutes($endTime) > config('config.attendance.early_leaving_grace_period')) {
            $isEarlyLeaving = true;
        }

        $totalWorkDuration = 0;
        foreach ($employeeTimesheets as $employeeTimesheet) {
            $totalWorkDuration += Carbon::parse($employeeTimesheet->out_at->value)->diffInMinutes($employeeTimesheet->in_at->value);
        }

        $options = [];
        if ($isLate) {
            $options['is_late'] = true;
            $options['late_duration'] = $firstInTime->diffInMinutes($startTime);
        }

        if ($isEarlyLeaving) {
            $options['is_early_leaving'] = true;
            $options['early_leaving_duration'] = $lastOutTime->diffInMinutes($endTime);
        }
        ...
        if ($totalWorkDuration >= $totalShiftDuration || ($totalShiftDuration - $totalWorkDuration <= config('config.attendance.present_grace_period'))) {
