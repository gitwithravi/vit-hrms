<template>
    <BaseModal :visibility="visibility" @close="emit('close')">
        <template #title>Download Timesheet Report</template>
...
const submit = () => {
    ...
    window.open(
        `/app/attendance/timesheets/report?start_date=${form.start_date}&end_date=${form.end_date}`,
        "_blank"
    )
    emit("close")
}
